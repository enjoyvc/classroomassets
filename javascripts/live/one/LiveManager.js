...
var P_S_mediaObj = {        //记录自己发布和订阅的资源ID
    video:{
...
var P_S_source = {              //存储所有发布的视频资源
    video:{
        publish:[]
    }
};
var isTimeout = false;
var recreateMeetingCount = 0;       //重新创建会议次数
var role;   //用户角色
var userid; //用户ID
var realName;//用户名称
...
LiveManager.prototype = {
            ...
            if(!LiveManagerModule.instance.getSharePCVideoStatus()){
                if(role == 1){
                    if(data.userid != userid){
                        ...
                        if(currentVideo === 't_1') {
                            if(LiveManagerModule.instance._isHavaSecondVideo())
                                LiveManagerModule.instance.leaveChange('t_2');
                        }else if(currentVideo === 't_2') {
    ...
    _addPublishVideoSourceId:function(data){            //存储发布视频的资源id
        LiveManagerModule.instance._removePublishVideoSourceId(data.userid);
        var obj = {};
        obj.userid = data.userid;
        obj.source_name = data.source_name;
        obj.source_id = data.source_id;
        obj.isTeacher = data.isTeacher;
        P_S_source.video.publish.push(obj);
    },
    _removePublishVideoSourceId:function(uid){          //移除发布视频的资源id
        if(!LiveManagerModule.instance._isOneself(uid)){
            var i = null;
            $.each(P_S_source.video.publish,function(index,obj){
                if(uid == obj.userid){
                    i = index;
                    return false;
                }
            ...
            if(i != null){
                P_S_source.video.publish.splice(i,1);
            }
        }
    },
    onReceive : function(data) {    // 接受到服务器端的回调
        ...
        if(data!=null){
            ...
            //    var msg = "您已经和服务器断开连接或者您的账号已经在异处登录,请重新进入";
                ...
                /*if(!LiveManagerModule.instance._isOneself(data.userid)) {
                    if(role == 1){
                        if(LiveManagerModule.instance.getTeacherControlAllVideoStatus() == 0)
                            LiveManagerModule.instance._video_publish_successCMDtoSub(data);        //订阅视频
                    }else{
                        LiveManagerModule.instance._video_publish_successCMDtoSub(data);        //订阅视频
                    }
                    LiveManagerModule.instance._addPublishVideoSourceId(data);
                }*/
                if(!LiveManagerModule.instance._isOneself(data.userid))
                    LiveManagerModule.instance._video_publish_successCMDtoSub(data);        //订阅视频
            }
            ...
            else if(data.cmd == "video:video_hangup_disallow"){
                console.log("------------------------------:挂机视频请求disabled");
            }
            else if(data.cmd == "video:video_hangup_success"){      //用户挂机视频成功
            ...
            else if(data.cmd == "video:audio_hangup_disallow"){
                console.log("------------------------------:挂机音频请求disabled");
            }
            else if(data.cmd == "video:audio_hangup_success"){      //用户挂机音频成功

            }
            /*else if(data.cmd == "video:video_hangup_success"){      //挂机视频成功
                LiveManagerModule.instance._removePublishVideoSourceId(data.userid);        //移除资源id
            }*/
...
//                LiveManagerModule.instance._changeCMDtoHandupOff(data);                   //关闭学生的举手状态
    ...
    setTeacherVideoOpenStatus:function(value,type){
        //TODO::记录老师摄像头开启状态
    },
    ...
    subscribeSipHangUpVideo:function(id){        //挂机订阅的视频
        var subObj = LiveManagerModule.instance._findMemberByUserid(id,P_S_mediaObj.video.subscribe);
        if(subObj != null && typeof subObj !== 'undefined') {
            var handupID = subObj.hid;
            if (handupID != null && typeof handupID !== 'undefined') {
                cfManager.subscribeSipHangUp(sipRoomNumber, handupID);
            }
        }
    },
    subscribeSipVideo:function(id){             //订阅视频-----未完成代码
        var _this = $('#'+id)[0];
        var idValue = id;
        var _w = s_t_s_w;
        var _h = s_t_s_h;
        var source_id = null;
        $.each(P_S_source.video.publish,function(index,obj){
            if(obj.userid == id && obj.source_name === 'video'){
                source_id = obj.source_id;
                return false;
            }
        });
        if(source_id != null)
            LiveManagerModule.instance._subscribe(_this, idValue, _w, _h,2, 'video', id, source_id);
    },
    ...
    _joinMeetingCallBack: function(iCode, iMsgInfo, oSession){       //加入会议室回调---ok
        if(iCode == ret_code.MEET_FAIL || iCode == ret_code.MEET_OTHER || iCode == ret_code.MEET_LIST || iCode == ret_code.MEET_NOT
            ...
            if(iCode == ret_code.MEET_JOIN && typeof(oSession) !== "undefined" && typeof(oSession.param) !== "undefined" && oSession.param.result == 0){    //加入会议室成功
            ...
            }else if(typeof(oSession) !== "undefined" && typeof(oSession.param) !== "undefined" && oSession.param.result == 5){//会议不存在
                ...
                console.debug("----------------->>>加入失败iCode="+iCode+",oSession="+oSession);
            }
        }
    },
        ...
        if(mediaObj.video_P.MEDIA_STATUS && mediaObj.video_P.SET_STATUS && mediaObj.video_P.OK_STATUS){
            ...
            if(role == 1){
                if(typeof oSession !== 'undefined' && oSession.labelInfo.id === 'divVideoLocal_a') {//判断为第一个视频
                    ...
                    if (LiveManagerModule.instance.getClassroomType() != 1) {
                        var src = $('#divVideoLocal_a video').attr('src');
                        $('#divVideoRemote #teacher_a').attr('src', src);
                    }
    ...
    _videoSubscribeSuccessFun: function(){      //视频订阅成功后执行的方法
        if(mediaObj.video_S.MEDIA_STATUS && mediaObj.video_S.SET_STATUS && mediaObj.video_S.OK_STATUS){
            ...
            if(role == "1" && LiveManagerModule.instance.getClassroomType() == 1){
                setTimeout(function(){
                    if(P_S_mediaObj.video.subscribe.length > 0) {
                        var suserid = P_S_mediaObj.video.subscribe[0].userid;
                        if (!LiveManagerModule.instance.getSharePCVideoStatus()) {
                            var ts = $('#' + suserid + ' video').attr('src');
                            $('#divVideoRemote video').attr('src', ts);
                            if (typeof(ts) === "undefined") {
                                $('#divVideoRemote video').attr('src', '');
                            }
    ...
    _sipCBMsgInfo:function(iCode, iMsgInfo, oSession) {  //回调函数
        ...
        LiveManagerModule.instance._videoSubscribeSuccessFun();
    ...
    _hangupFailCallBack:function(iCode, iMshInfo, oSession){        //挂机失败
