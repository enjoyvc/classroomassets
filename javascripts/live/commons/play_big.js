...
$(function(){
	...
	$(".student-list").css("height",$(window).height()-555);
...
//        var hh = $(".main").height() - 34;
    ...
    /*---录屏--*/
    $('.nav-bar span').each(function(i){
        $(this).click(function(){
            //$(this).addClass('on').siblings().removeClass('on');
            //$('.prv-box').eq(i).show().siblings().hide();
        })
    })
    $('.prv-box .choose span').click(function(){
        $(this).addClass('on').siblings().removeClass('on');
		rec.setInput($(this).attr("id"));
    })
    $('.prv-ico').each(function(){
         if($('.pop-employ').is(":hidden")){
            $('.prv-ico').removeClass('prv-ico-on');
        }else{ $('.prv-ico').addClass('prv-ico-on');}
		/*
        $(this).click(function(){
			if(rec.showRecord() == true) {
				if($('.pop-employ').is(":hidden")){
					$('.prv-ico').addClass('prv-ico-on');
				}else{ $(this).removeClass('prv-ico-on');}
			   
				$('.pop-employ').toggleClass('none');
			}
			else {
				rec.showOrHideStatus();
			}
        })
		*/
    })
