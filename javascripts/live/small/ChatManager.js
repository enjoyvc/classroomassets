...
ChatManager.prototype = {
    ...
    //--由liveController实现的方法区--end
		...
		_parseJSON:function(event){
            ...
            if(data.cmd == "robot:talk"){
                /*var json = { "cmd":"robot:reply","roomid":ChatManagerModule.instance.classRoomController.classRoomID,"userid":param };
                ChatManagerModule.instance._sendMessage(json);*/
            }
            if (data!=null) {
            ...
            if (variable !== null && variable !== undefined && variable !== '' && data.cmd == "chat:privatetalk") {
                ...
                if(data.cmd == "classroom:join"){
                    ...
                    if(data.userid == param && !data.isteacher && data.classroom.quota > 10 && data.classroom.ispublic && data.isauth == 0){
                        ...
                        if(role == 2) {
                            if (data.controlpanel.switch_discuss_zone_permission==0)
                                publicChatFlag = true;
                            else
                                publicChatFlag = false;
                        }
            ...
            }else if(data.cmd == "controlpanel:switch_discuss_zone_permission"){
                if(role == 2) {
                    if (data.status == 0)
                        publicChatFlag = true;
                    else
