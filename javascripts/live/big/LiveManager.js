...
LiveManager.prototype = {
                            ...
                            if (data.videoName == "video") {
                                if (currentVideo == 't_1') {
                                    if (typeof (MCU_VER) === 'undefined') {
                                        ...
                                        $('#teacher_a').prop('srcObject', null);
                                        var bsrc = $('#divVideoLocal_b video').prop('srcObject');
                                    }
                                ...
                                if (currentVideo == 't_2') {
                                    if (typeof (MCU_VER) === 'undefined') {
                                        ...
                                        $('#teacher_a').prop('srcObject', null);
                                        var asrc = $('#divVideoLocal_a video').prop('srcObject');
                                    }
        ...
        if(true) {
            if (!LiveManagerModule.instance.classroomInfoManager.getTeacherShareVideo()) {		//如果老师没有打开共享屏幕
                if (typeof (MCU_VER) === 'undefined')
                    ...
                    var src = $('#' + data.videoid + ' video').prop('srcObject');
                ...
                if (typeof (MCU_VER) === 'undefined')
                    ...
                    $('#divVideoRemote video').prop('srcObject', src);
    ...
    _changeCMDtoChangeMainFrame : function(data,ts){                //改变主画面的显示
        ...
        if (LiveManagerModule.instance.userManager.isTeacher()) {		//如果自己是老师直接切换
            if (typeof (MCU_VER) === 'undefined') {
                ...
                sourceSrc = $('#' + ts + ' video').get(0).srcObject;
                $('#divVideoRemote video').get(0).srcObject = sourceSrc;
            }
            if(typeof(sourceSrc) === "undefined"){
                if (ts !== 'divVideoLocal_a' && ts !== 'divVideoLocal_b') {
                    if (typeof (MCU_VER) === 'undefined') {
                        ...
                        sourceSrc = $('#search_' + ts + ' video').prop('srcObject');
                        if (typeof (sourceSrc) !== 'undefined')
                            $('#divVideoRemote video').prop('srcObject', sourceSrc);
                    }
                }
                if (typeof (MCU_VER) === 'undefined')
                    $('#divVideoRemote video').attr('src', '');
                else
                    $('#divVideoRemote video').prop('srcObject', null);               
            }
            LiveManagerModule.instance.classroomInfoManager.teacherCurrentVideo = data.videoid;
            ...
            if(data.videoid !== 't_1' && data.videoid !== 't_2' &&
                ...
                if (!LiveManagerModule.instance.classroomInfoManager.getTeacherShareVideo()) {		//如果老师没有打开共享屏幕
                    if (typeof (MCU_VER) === 'undefined') {
                        ...
                        sourceSrc = $('#' + ts + ' video').prop('srcObject');
                        $('#teacher_a').prop('srcObject', sourceSrc);
                    }
            		LiveManagerModule.instance.classroomInfoManager.teacherCurrentVideo = data.videoid;
            	}
            }
                ...
                if(!LiveManagerModule.instance.userManager.isOneselfByUserid(data.userid)){
                    ...
                    if(LiveManagerModule.instance.classroomInfoManager.getTeacherWhiteboardStatus()){
                        ...
                        if (typeof (MCU_VER) === 'undefined') {
                            ...
                            var cvideo = $('#' + ts + " video").prop('srcObject');
                            $('#teacher_a').prop('srcObject', cvideo);
                            if (typeof (cvideo) === "undefined") {
                                $('#teacher_a').prop('srcObject', null);
                            }
                        }
    ...
    _audioSubscribe:function(user,source_id){       //订阅音频
        ...
        var tag;
        if (typeof (MCU_VER) === 'undefined') {
            phoneNumber = LiveManagerModule.instance.sipRoomNumber;
            tag = document.getElementById('audioRemote');
        }
        else {
            tag = cfManager.sipCreateAudioTag("audio_div", PublicStatic.StaticClass.tsk_string_random(3));
            phoneNumber = source_id;
        }

        
        
        var hangupID = cfManager.sipSubscribeCall('call-audio', LiveManagerModule.instance.sipRoomNumber, [{
            call_PhoneNumber: phoneNumber,        //一个会议对应一个音频，source_id取会议id
            audio_remote: tag,
            video_local: null,
            video_remote: null,
            audio_mode:'2'
    ...
    _isHaveFirstVideo: function () {
        if (typeof (MCU_VER) === 'undefined')
            ...
            var src = $('#divVideoLocal_a video').prop('srcObject');
        if(typeof src !== 'undefined' && src != null){
            return true;
        }
        return false;
    ...
    _isHavaSecondVideo: function () {
        if (typeof (MCU_VER) === 'undefined')
            ...
            var src = $('#divVideoLocal_b video').prop('srcObject');
        if(typeof src !== 'undefined' && src != null){
            return true;
        }
        return false;
            ...
            if(!LiveManagerModule.instance.userManager.isTeacher()){
                ...
                setTimeout(function(){
                    ...
                    if (typeof (MCU_VER) === 'undefined') {
                        ...
                        ts = $('#stuShareVideo video').prop('srcObject');
                        $('#divVideoRemote video').prop('srcObject', ts);
                        if (typeof (ts) === "undefiend") {
                            $('#divVideoRemote video').prop('srcObject', null);
                        }
                    }
                    ...
                    if(videoid == "t_1"){                   //如果老师主画面为第一个视频
                        setTimeout(function () {
                            if (typeof (MCU_VER) === 'undefined') {
                                ...
                                ts = $('#divVideoLocal_a video').prop('srcObject');
                                if (ts != $('#divVideoRemote video').prop('srcObject')) {
                                    $('#divVideoRemote video').prop('srcObject', ts);
                                }
                                if (typeof (ts) === "undefiend") {
                                    $('#divVideoRemote video').prop('srcObject', null);
                        ...
                        setTimeout(function () {
                            if (typeof (MCU_VER) === 'undefined') {
                                ...
                                ts = $('#divVideoLocal_b video').prop('srcObject');
                                if (ts != $('#divVideoRemote video').prop('srcObject')) {
                                    $('#divVideoRemote video').prop('srcObject', ts);
                                }
                                if (typeof (ts) === "undefined") {
                        ...
                        setTimeout(function(){
                            if (LiveManagerModule.instance.classroomInfoManager.getTeacherCurrentVideo() == videoid) {
                                if (typeof (MCU_VER) === 'undefined') {
                                    ...
                                    var t = $('#' + videoid + ' video').prop('srcObject');
                                    if (t != $('#divVideoRemote video').prop('srcObject')) {
                                        $('#divVideoRemote video').prop('srcObject', t);
                                    }
                                }
                            	return;                            		
                            }
                            if (typeof (MCU_VER) === 'undefined') {
                                ...
                                ts = $('#currentVideo video').prop('srcObject');
                                if (ts != $('#divVideoRemote video').prop('srcObject')) {
                                    $('#divVideoRemote video').prop('srcObject', ts);
                                }
                                if (typeof (ts) === "undefined") {
                ...
                if(htmlId == 'handupVideo'){
                    setTimeout(function () {
                        if (typeof (MCU_VER) === 'undefined') {
                            ...
                            ts = $('#handupVideo video').prop('srcObject');
                            $('#divVideoRemote video').prop('srcObject', ts);
                            if (typeof (ts) === "undefiend") {
                                $('#divVideoRemote video').prop('srcObject', null);
                            }
