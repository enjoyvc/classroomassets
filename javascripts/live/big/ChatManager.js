...
KindEditor.ready(function(K) {
	if(typeof(globalUI) != "undefined") {
        options.cssData = '.ke-content{padding:0 0 0 10px; font:500 12px/30px \"PingFang SC\"; color:#666;}.ke-content p{margin:0;}'
        ; 
	}
...
ChatManager.prototype = {
    ...
    setController : function(classRoomController) {
        ChatManagerModule.instance.classRoomController = classRoomController;
        ChatManagerModule.instance.classroomInfoManager = classRoomController.getClassroomInfoManager();      //房间信息对象
        ChatManagerModule.instance.userObj = classRoomController.getUserManager();        //用户对象
    ...
    sendAdMessage: function() {
        ...
        if (!ChatManagerModule.instance.userObj.isTeacher()) {
            var msg = VC.ROUTER.LIVEBIG.CHATMANAGER_STUDENT_GAG;
            ChatManagerModule.instance.classroomInfoManager.showMessageWindow(msg);
            return false;
        }
        var imgLink = $("#imglink").attr("value");
        var prodLink = $("#prodlink").attr("value");
        if (imgLink.length == 0 || prodLink.length == 0)
            return false;
        var msg = '{"img": "' + imgLink  + '", "prod": "' + prodLink + '"}';
    ...
    sendPublicMessage : function(event){//发送公聊信息
        ...
        ChatManagerModule.instance._sendMessagePu(event);//公聊
    },
    _sendMessagePu:function(event){//公聊
        editor.sync();
        var messagejson = new Object();
        ...
        //    msg = ChatManagerModule.instance.adMessage(msg);
        //}
        messagejson.message = msg;
        messagejson.classroomName = ChatManagerModule.instance.classroomInfoManager.getClassroomName();
        ChatManagerModule.instance.classRoomController.sendMessage(messagejson);
    ...
    */
    ...
    handleAdMessage: function (msg) {
        if (msg[0] != '{' || msg[msg.length - 1] != '}')
            return false;
        //var reg = /<ad><\/ad>/;
        //if (!reg.test(msg)) return false;
        //msg = msg.replace(reg, '');
    ...
    onReceive:function(event){          //接收消息
        ...
        if (PublicStatic.StaticClass.validateByValue(data.touserid) && data.cmd == "chat:privatetalk"){ //私聊
            ...
            if (ChatManagerModule.instance.handleAdMessage(data.message))
                return;
