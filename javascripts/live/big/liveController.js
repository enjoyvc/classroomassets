classRoomModule.controller('liveController',['$scope','liveFactory','$log','$sce','$interval','$timeout','$modal',
    function($scope,liveFactory,$log,$sce,$interval,$timeout,$modal){
        ...
        var chatManager = new ChatManager(this);
    ...
    function liveControllerInit() {        
        ...
        classRoomControler.init(chatManager);
        ...
        var _userIsCanChangeVideo = function(value){                  //判断能否进行切换
            ...
            if (value == 't_1') {              //判断要切换的是否为老师的第一个视频
                if (typeof (MCU_VER) === 'undefined')
                    ...
                    vsrc = $('#divVideoLocal_a video').get(0).srcObject;
            } else if (value == 't_2') {        //判断要切换的是否为老师的第一个视频
                if (typeof (MCU_VER) === 'undefined')
                    vsrc = $('#divVideoLocal_b video').attr('src');
                else
                    vsrc = $('#divVideoLocal_b video').get(0).srcObject;
            }else{
                ...
                if(obj != null){
                    ...
                    if (typeof (MCU_VER) === 'undefined')
                        ...
                        vsrc = $('#' + uid + ' video').get(0).srcObject;
                }
            }
        ...
        var _userIsCanChangeVideoBySearch = function(value){        //判断能进行切换--搜索
            ...
            if(obj != null){
                ...
                if (typeof (MCU_VER) === 'undefined')
                    ...
                    vsrc = $('#search_' + uid + ' video').prop('srcObject');
            }
            if(typeof vsrc === 'undefined' || vsrc == null){
                return false;
            }
        ...
        var _subscribeCarouseVideo = function(){                    //订阅走马灯前show个学生视频
            ...
            $.each(classroomInfoManager.getMembers(),function(index,member){
                if(number <= classroomInfoManager.getScrollMembersCount()){
                    if(member.roleid == 2 && !userManager.isOneselfByUserid(member.userid)){
                        _subscribeVideo(member.userid);
                        number++;
                    }
                }
            });
        ...
        vm.teacherInfo.selectStudent = function(){                  //模糊查询学生
            ...
            if(vm.classroomInfo.searchKey != null && vm.classroomInfo.searchKey !== ""){
                if(findResult.length > 0) {
                    idObject = $('#'+ findResult[0].userid)[0];
                    idObject.focus();
                }
        ...
        vm.teacherInfo.setGuestInMain = function() {
            if(userManager.isGuest()) {
                userManager.clearGuest();
                if(userManager.getOpenFirstVideoStatus() == false)
                    vm.firstVideoOnAndOff();
                //userManager.openOrCloseWhiteboard();
                liveManager.changeVideoShow('t_1');
                return;
            }
            var userid = classroomInfoManager.getTeacherCurrentVideo();
            if(userid != "t_1" && userid != "t_2" && userid != null) {
                var index = classroomInfoManager.findIndexFromMembersById(userid);
                if(index != null) {
                    m = classroomInfoManager.members[index];        
                    userManager.setGuest(m);
                    userManager.openOrCloseWhiteboard();
                    return;
                }
            }
            var msg = "请先选择嘉宾视频";//VC.ROUTER.LIVEBIG.NO_THIS_VIDEO;
        ...
        var _findFirstMember = function(){     //获取members里面的第一个学生userid
            var id = null;
            $.each(classroomInfoManager.getMembers(),function(index,member){
                if(member.roleid == 2 &&　!userManager.isOneselfByUserid(member.userid)){
                    id = member.userid;
        ...
        var _hangupVideo = function(id){     //挂机视频
        	var obj = classroomInfoManager.findObjectFromMembersById(id);
        ...
        var _moveFirstStudentToLast = function(id){    //移动第一个学生至最后    	    	
            var tempObj = classroomInfoManager.findObjectFromMembersById(id);
            if(tempObj != null){
                classroomInfoManager.deleteUserFromMembersByUserid(id);
                classroomInfoManager.addUserToMembersByObject(tempObj);
            }
        };
        var _findNextMember = function(){                //获取members里面的下一个要显示的学生的userid
            var id = null;
            var num = 0;
            $.each(classroomInfoManager.getMembers(),function(index,member){
                if(member.roleid == 2 && !userManager.isOneselfByUserid(member.userid)){
        ...
        var _carouseVideo = function(){
            ...
            var _id = _findFirstMember();
    //        if(userManager.isTeacher()) {
                _hangupVideo(_id);
    //        }
            _moveFirstStudentToLast(_id);
    //        if(userManager.isTeacher()) {
                var _nextId = _findNextMember();
                $timeout(function () {
                    _subscribeVideo(_nextId);
                }, 1000);
    //        }
        };
        ...
        vm.sendPublicMessage = function (event) {  //发送公聊信息       //teacher---student--->>>view
            chatManager.sendPublicMessage(event);
        };
        vm.sendAdMessage = function () {
            chatManager.sendAdMessage();
        }
        ...
        vm.firstVideoOnAndOff = function(){//老师开关第一个视频  //teacher--->>>view
            if(userManager.isGuest()) {
                var msg = "请先停止嘉宾功能";//VC.ROUTER.LIVEBIG.NO_THIS_VIDEO;
                classroomInfoManager.showMessageWindow(msg);
                return;
            }
            if(_videoPromise())
        ...
        vm.isShowPageOperation = function(){//是否显示分页操作  teacher---student--->>>view
            return (userManager.isTeacher() || userManager.isGuest()) && whiteBoardManager.totalPage > 0;
        };
        ...
        var getVideoAPIParam = function(callback){
            //David edit
            var url = "https://vccamp.vccore.com" + "/classroom/room/param" + window.location.search;
            ...
            liveFactory.get(url).then(function(data){
                if(data.success){
                    ...
                    $('#vcliveHost').attr("value", "http://vccamp.vccore.com");    
