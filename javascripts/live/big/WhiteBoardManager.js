...
WhiteBoardManager.prototype = {
    ...
    getIsOpenWhiteboard: function () {//获取白板是否打开状态
        var bigWindow;
        var smallWindow;
        var isOpen = this.isOpenWhiteboard;
        
        if (typeof (globalUI) !== 'undefined') {
                if (typeof (MCU_VER) === 'undefined') {
                    bigWindow = $('#divVideoRemote video').attr('src');
                    smallWindow = $('#divVideoLocal_a video').attr('src');
                }
                else {
                    bigWindow = $('#divVideoRemote video').prop('srcObject');
                    smallWindow = $('#divVideoLocal_a video').prop('srcObject');
                }
                if (bigWindow === smallWindow) {
                    isOpen = true;
                }
        }
        
		if(typeof(globalUI) != "undefined" && isOpen) {
			b = $('#back-canvas').offset();
			$('#top-canvus').offset(b);
			$('#show-drawing-canvus').offset(b);
			$('#drawing-canvus').offset(b);
			//setTimeout(function(){LiveManagerModule.instance.applyData();}, 100);
		}
		return isOpen;
    },
...
// ==================== start 辅助私有方法 ===================
	...
	_loadBackCanvasImgae:function(imageSrc){
		...
		this._preImage(imageSrc,function(imgs){
			canvas_back.width = imgs.width;
			canvas_back.height = imgs.height;
		...
		if(isshow){
            ...
            if(WhiteBoardManagerModule.instance.userManager.isTeacher()) {
                ...
                if(WhiteBoardManagerModule.instance.getIsOpenShareWhiteboard()){
                    WhiteBoardManagerModule.instance.whiteboardToolsOn();
                } else if(WhiteBoardManagerModule.instance.userManager.isGuest()) {
                 	WhiteBoardManagerModule.instance.whiteboardToolsOn();
            	}else{
...
//		    var imageData = canvas_color_context.getImageData(e.pageX - canvas_color.offsetLeft, e.pageY - canvas_color.offsetTop, canvas_color.width, canvas_color.height);
	...
	_bindToolsEvent:function(){
		...
		$("#changeLine li").bind("click", function(){           //选择线条
            ...
            WhiteBoardManagerModule.instance.currentLineValue = parseInt(classValue);
...
//					$(drawingCanvas).trigger("draw", tempXY);
	...
	_bindPageEvent:function(first, prev, next, last){
		...
		$(first).click(function () {
			...
			WhiteBoardManagerModule.instance.oldPage = WhiteBoardManagerModule.instance.currentPage;
            WhiteBoardManagerModule.instance.currentPage = 1;

			var firstSrc = parent._getImageSrc(WhiteBoardManagerModule.instance.currentPage);
			if(firstSrc != "")
		...
		$(prev).click(function () {
			if(WhiteBoardManagerModule.instance.currentPage == 1){
                var msg = VC.ROUTER.LIVEBIG.WHITEBOARDMANAGER_FIRST_PAGE;
                WhiteBoardManagerModule.instance.classroomInfoManager.showMessageWindow(msg);
				return;
			}
			var drawCanvas = $("#drawing-canvus");
		    var image = drawCanvas[0].toDataURL("image/png");
            WhiteBoardManagerModule.instance.currentPage = WhiteBoardManagerModule.instance.currentPage - 1;
			var prevSrc = parent._getImageSrc(WhiteBoardManagerModule.instance.currentPage);
			if(prevSrc != "")
		...
		$(next).click(function () {
			if(WhiteBoardManagerModule.instance.currentPage == WhiteBoardManagerModule.instance.totalPage || WhiteBoardManagerModule.instance.totalPage == 0){
		    ...
		    var image = drawCanvas[0].toDataURL("image/png");
            WhiteBoardManagerModule.instance.currentPage = WhiteBoardManagerModule.instance.currentPage + 1;
			var nextSrc = parent._getImageSrc(WhiteBoardManagerModule.instance.currentPage);
			if(nextSrc != "")
		...
		$(last).click(function () {
			...
			WhiteBoardManagerModule.instance.oldPage = WhiteBoardManagerModule.instance.currentPage;
            WhiteBoardManagerModule.instance.currentPage = parseInt(WhiteBoardManagerModule.instance.totalPage);
			var drawCanvas = $("#drawing-canvus");
		    var image = drawCanvas[0].toDataURL("image/png");
			var lastSrc = parent._getImageSrc(WhiteBoardManagerModule.instance.currentPage);
			if(lastSrc != "")
